...
public class hot04 {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {    //暴力解法，先合并，再找中位数
        int[] nums;
        int m = nums1.length;
        int n = nums2.length;
        nums = new int[m + n];
    ...
    public double findMedianSortedArrays2(int[] A, int[] B) {       //只需要遍历一半即可。
        int m = A.length;
        int n = B.length;
        int len = m + n;
        int left = -1, right = -1;            //存储上次和现在的数
        int aStart = 0, bStart = 0;
        for (int i = 0; i <= len / 2; i++) {
            left = right;
            if (aStart < m && (bStart >= n || A[aStart] < B[bStart])) {    //防止越界
                right = A[aStart++];
            } else {
                right = B[bStart++];
            }
    ...
    public double findMedianSortedArrays3(int[] nums1, int[] nums2) {      //时间复杂度为o(log(m+n))
        int n = nums1.length;
        int m = nums2.length;
